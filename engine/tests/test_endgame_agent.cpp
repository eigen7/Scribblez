// EndgameAgent<HastyBotAgent>: HastyBot while the bag holds tiles, the endgame solver
...
EndgameAgent<HastyBotAgent>::Params endgame_params(uint64_t budget, int plies) {
  EndgameAgent<HastyBotAgent>::Params p;
  p.base = HastyBotAgent::Params{.thread_id = 0, .name = "EndgameHastyBot"};
...
TEST(EndgameAgent, PreEndgameDelegatesToHasty) {
  ...
  EndgameAgent<HastyBotAgent> eg(endgame_params(50000, 25));
...
TEST(EndgameAgent, EndgameTakeoverVsGreedy) {
  ...
  for (int i = 0; i < 1200 && !found; ++i) {
    ...
    EndgameAgent<HastyBotAgent> solving(endgame_params(kSolveBudget, kSolvePlies));
    EXPECT_EQ(solving.make_move(req).move, r.best);
    EXPECT_NE(solving.make_move(req).move, greedy);

    EndgameAgent<HastyBotAgent> disabled(endgame_params(/*nodes=*/0, kSolvePlies));
...
TEST(EndgameAgent, ShallowSolveFallsBackToHasty) {
  ...
  EndgameAgent<HastyBotAgent> tiny(endgame_params(/*nodes=*/1, kSolvePlies));
  HastyBotAgent hasty({.thread_id = 0, .name = "HastyBot"});
  int checked = 0;
  for (int i = 0; i < 30; ++i) {
    const EndgamePos p = random_endgame(rng, d, /*rack_tiles=*/3);
...
TEST(EndgameAgent, EndgameMoveIsLegal) {
  ...
  EndgameAgent<HastyBotAgent> agent(endgame_params(20000, 20));
  int checked = 0;
  for (int i = 0; i < 60; ++i) {
    const EndgamePos p = random_endgame(rng, d, /*rack_tiles=*/3);
    const MoveRequest req = endgame_request(p, d);
...
TEST(EndgameAgent, FullGameTinyDict) {
  ...
  for (uint64_t seed = 1; seed <= 8; ++seed) {
    EndgameAgent<HastyBotAgent> a0(endgame_params(20000, 25));
...
TEST(EndgameAgent, FullGameRealLexicon) {
  ...
  for (uint64_t seed = 1; seed <= 6; ++seed) {
    EndgameAgent<HastyBotAgent> a0(endgame_params(50000, 25));
    HastyBotAgent a1({.thread_id = 0, .name = "HastyBot"});
    Game g(a0, a1, d, seed);
    g.play();
    check_log_consistency(g.log());
...
TEST(EndgameAgent, FromSpecParsing) {
  ...
  EXPECT_NE(EndgameAgent<HastyBotAgent>::from_spec(
              {"--endgame-budget=1234", "--endgame-plies=7", "--temperature=0"}, 0, "X"),
            nullptr);
  EXPECT_NE(
    EndgameAgent<HastyBotAgent>::from_spec({"--top-k=5", "--temperature=1.5", "--seed=42"}, 0, "Y"),
    nullptr);
  ...
  const EndgamePos p = random_endgame(rng, d, /*rack_tiles=*/3);
  const MoveRequest req = endgame_request(p, d);
  auto disabled = EndgameAgent<HastyBotAgent>::from_spec({"--endgame-budget=0"}, 0, "Z");
  EXPECT_EQ(disabled->make_move(req).move, hasty_best_move_wmp(req));

  EXPECT_THROW(EndgameAgent<HastyBotAgent>::from_spec({"--endgame-plies=notanint"}, 0, "B"),
               std::runtime_error);
  EXPECT_THROW(EndgameAgent<HastyBotAgent>::from_spec({"--bogus-option=1"}, 0, "C"),
               std::runtime_error);
}
...
TEST(EndgameAgent, SpreadMattersFromSpec) {
  ...
  EXPECT_NE(EndgameAgent<HastyBotAgent>::from_spec(
              {"--endgame-spread-matters=1", "--endgame-budget=777"}, 0, "W"),
            nullptr);
  EXPECT_NE(EndgameAgent<HastyBotAgent>::from_spec({"--endgame-spread-matters=0"}, 0, "X"),
            nullptr);
  EXPECT_NE(EndgameAgent<HastyBotAgent>::from_spec({"--endgame-budget=777"}, 0, "V"), nullptr);
  EXPECT_THROW(EndgameAgent<HastyBotAgent>::from_spec({"--endgame-spread-matters=maybe"}, 0, "B"),
               std::runtime_error);
}
...
TEST(EndgameAgent, FirstWinProjectsCertificates) {
  ...
  EndgameSolver ref;
  HastyBotAgent hasty({.thread_id = 0, .name = "HastyBot"});
  EndgameAgent<HastyBotAgent>::Params wp = endgame_params(kSolveBudget, kSolvePlies);
  wp.solver.spread_matters = false;
  EndgameAgent<HastyBotAgent> wld(wp);
...
TEST(EndgameAgent, FastTrackReducesPrompts) {
  ...
  for (int mode = 0; mode < 2; ++mode) {
    ...
    EndgameAgent<HastyBotAgent>::Params params = endgame_params(1600, 25);
    params.solver.spread_matters = false;
    EndgameAgent<HastyBotAgent> inner0(params), inner1(params);
