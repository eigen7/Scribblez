...
// Move-feature encoding for the pre-move value model (M_pre). The board is
...
// reaches it through the FFI (scribblez_pre_move_value_encode_moves) and the
// M_pre agent encodes candidates the same way at inference, so the two never
// drift. The placement is a pure function of the Move -- a PLAY's placed
// squares come from visit_placed_squares and its tiles from Move::glyph in the
// same lane order; EXCHANGE/PASS place nothing and differ only in the scalars.
//
...
// that is the quantity M_post evaluates, encoded on the same scale as the board
// trunk's score-diff input (input_encoder.h), so the two are directly
// comparable. The leave is intentionally not encoded -- it is recoverable from
// the mover's rack, which the board trunk sees, minus the placed tiles.
