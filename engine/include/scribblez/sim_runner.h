...
// mover's own rack. The opponent's rack is unknown and is re-sampled per
// rollout from the unseen pool (a full bag minus the board tiles and `rack`).
struct SimPosition {
  Board board;
...
class SimRunner {
 public:
  ...
  // every candidate is seeded by `base_seed + i` (the CRN scheme above), and
