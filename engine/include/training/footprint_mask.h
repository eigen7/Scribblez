...
// opponent's tile supply) -- no move-gen, no dictionary lookup on the main word,
// no joint multi-cell tile contention.
...
// both orientations at once -- i.e. legal in the vertical AND the horizontal
// cross-word together (a neighbour-free axis constrains nothing). So a tile below
// a vertical word whose only end-hook is unavailable is masked, and a cross-point
// whose two cross-words share no legal letter is illegal even though each axis
// alone admits one.
//
...
// minus the mover's rack. A cell's cross-check is intersected with the letters
// `supply` has in stock; a blank in `supply` is a wildcard that satisfies any
// board-legal square. Passing `supply == nullptr` disables availability (every
// letter treated as in stock), recovering the pure board-legality mask.
//
...
// unseen pool, so any letter it truly plays is in `supply` (with sufficient
// count, as its tiles are unseen to us). The mask still omits the main-word
// dictionary check and joint multi-cell tile contention, so it can admit
// footprints no real move realizes (the model learns those toward zero); see
// the contention follow-up for tightening the per-cell test to a joint one.
//
