...
// then the blank in `hi`), supporting an O(1) subset test. Rack and per-move
// tile counts never exceed RACK_SIZE, so every field's high bit is free to act
// as the borrow sentinel in counts_subset's SWAR subtraction.
struct PackedCounts {
  uint64_t lo = 0;
  uint64_t hi = 0;
};
...
// The lane-touch set of playing `m` on `board` (the board as it stands before
// `m`). It is the exact dual of the out-play halo (see outplays.h), coarsened
// to lanes: a move can change legality, score, or the single-tile dedup of a
// previously legal play -- or enable a new play -- only in these lanes.
//
...
// that is the cell from which a word extends through p's run; perpendicular,
// the cell whose cross-word p's run rewrites. Marking the rows and columns of
// p and of those (at most four) cells therefore covers every affected play,
// and every newly enabled play lies in a marked lane as well (it must place on
// such a cell or play through p). Cross-check, anchor, and dedup state in
// unmarked lanes is untouched, so their move sets carry over verbatim --
// including scores.
...
// every move made (with the board still in its pre-move state), and request
// moves_at(ply) only after every shallower ply of the current path was
// materialized by the same means -- the natural shape of a depth-first search,
// where each node generates before descending. Slots are indexed by ply and
// overwritten freely as sibling subtrees revisit a depth.
class PathMoveLists {
 public:
  // Bind the search's board and dictionary and size the per-ply slots;
  // `max_ply` bounds the plies on_make/moves_at will see.
  void reset(const Board* board, const Dictionary* dict, int max_ply);

  // Seed side `side`'s legal plays on the root board (0 is the side to move at
  // the root, 1 the opponent), as generated by MoveGenerator::generate.
  void set_root_list(int side, const std::vector<Move>& plays);

  // Record the move about to be made at `ply`; `board` must still be in its
  // pre-move state.
  void on_make(int ply, const Move& m);

  // The side-to-move's legal plays at the current position, `ply` deep down
  // the path, generated against `rack` (that side's current rack).
 ...
 private:
  ...
  // [lane_begin[L], lane_begin[L + 1])), with each move's used-tile multiset
  // packed alongside for the rack-subset filter.
  struct Slot {
