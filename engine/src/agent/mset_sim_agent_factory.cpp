...
std::unique_ptr<MsetSimAgent> MsetSimAgent::from_spec(const std::vector<std::string>& tokens,
  ...
  if (!opts.leaf_model.empty()) {
    // The leaf net shares the service's device and precision; its per-call
    // ceiling is the position family's own (the runner batches to it).
    ...
    leaf_params.precision = nn::parse_precision(opts.service.precision);
