...
// Score moments are scaled to roughly unit range, and the rollout count
// through log1p and its own scale. Both must match evidence.py, or the fusion
...
// `cell`, and the catch-all classes are dropped.
template <typename T>
...
void stage_evidence(std::span<const Move> moves, std::span<const SimObservation> observations,
  ...
  // The fusion stage masks out padding regardless; zeroing it keeps the
  // inputs reproducible.
