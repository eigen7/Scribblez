"""Tests for the pre-move value model training side: the move-feature encoder
(pure CPU) and the end-to-end dataset -> train step -> recall eval path over a
tiny generated .pmt/.slog corpus (GPU, since the target generator builds a
TensorRT engine).
"""
...
import pytest
import torch
from scribblez.pre_move_value import moves as move_enc
...
def corpus_dir(tmp_path_factory) -> Path:
    """A tiny .slog corpus labeled with .pmt targets from a small teacher."""
