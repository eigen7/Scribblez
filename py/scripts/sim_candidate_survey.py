...
ranking -- PR 0 of docs/plans/sim_labeled_candidates.md. A pick outside the cut
counts only when an independent second sim puts it at least two standard
errors above the cut's own pick.
...
Each .slog gets a .simsurvey.json sidecar carrying, per candidate and replica,
the outcome counts, the final-margin histogram and both sides' next-move score
statistics -- enough for a later tool to classify why an outside play wins.
...
def main() -> int:
    ...
    setups_only = args.recipe == "setup"
    print(report(survey, args.cut, setups_only))
    if args.review_dir:
        command = shlex.join(["./py/scripts/sim_candidate_survey.py", *sys.argv[1:]])
        write_review_dir(survey, args.cut, setups_only, gcg_dir(args), args.review_dir, command)
    return 0
