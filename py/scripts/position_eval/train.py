#!/usr/bin/env python3
"""Trainer for the position evaluation model: an open-ended generate->train loop.
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Generational generate->train loop for the position evaluation model.",
    ...
    # Per-checkpoint evaluation over the committed position-evaluation GCG dataset
    ...
    p.add_argument(
        "--eval-dataset",
        type=str,
        default=str(position_eval_analysis.DEFAULT_DATASET),
    ...
    p.add_argument(
        "--no-eval",
        action="store_true",
        help="Disable the per-checkpoint position-evaluation dataset evaluation.",
    )
    p.add_argument(
        "--quality-dataset",
        type=str,
        default=str(position_eval_analysis.LARGE_DATASET),
...
def load_position_eval(args, spatial_planes: int) -> dict | None:
    ...
    if args.no_eval:
        return None
    try:
        names, inputs = position_eval_analysis.load_inputs(args.eval_dataset)
    except Exception as e:  # missing lexicon / unreadable dataset
        timed_print(f"position-evaluation eval disabled: {e}")
        return None
    if not names:
        timed_print(f"position-evaluation eval disabled: no GCG positions in {args.eval_dataset}")
        return None
    timed_print(f"position-evaluation eval: {len(names)} positions from {args.eval_dataset}")
...
def load_position_eval_quality(args, spatial_planes: int) -> dict | None:
    ...
    if args.no_quality:
        return None
    try:
        names, inputs = position_eval_analysis.load_inputs(args.quality_dataset)
        gt = position_eval_analysis.load_ground_truth(args.quality_dataset, names)
    except Exception as e:  # missing lexicon / dataset / ground truth
        timed_print(f"position-evaluation quality eval disabled: {e}")
        return None
    if not names:
        timed_print(
            f"position-evaluation quality eval disabled: no positions in {args.quality_dataset}"
        )
        return None
    timed_print(
        f"position-evaluation quality eval: {len(names)} positions from {args.quality_dataset}"
    )
...
def main() -> int:
    ...
    ctx = {
        ...
        "spatial_planes": spatial_planes,
        "scalar_size": scalar_size,
        "position_eval": load_position_eval(args, spatial_planes),
        "position_eval_quality": load_position_eval_quality(args, spatial_planes),
    }

    if not args.no_dashboard:
        for proc in react_server.spawn(
            "position_eval", str(paths.mount_root), dev_port=args.dashboard_port, tag=args.tag
        ):
