...
accumulated by scripts/generate_kill_test_data.py under the same tag:
  ...
  Passing --open-rack selects the open-rack information condition end to end:
  the cache decodes rows with the opponent-rack input block, and the .sobs
  sidecars are required to carry the open-rack flag (i.e. generated by
  generate_kill_test_data.py --open-rack, whose sims start the opponent from
  their true rack). Open-rack and hidden-rack artifacts must live under
  different tags; the cache records its mode and refuses a mismatch.
...
from scribblez.ffi import decode_rows, get_input_shapes, set_opp_rack_input
...
def build_cache(slog_dir: Path, cache: Path, holdout_every: int, max_k: int, open_rack: bool):
    ...
    mode = {"open_rack": open_rack}
    if mode_path.exists() and json.loads(mode_path.read_text()) != mode:
        raise SystemExit(f"{mode_path} disagrees with --open-rack; use a separate tag per mode")
    ...
    for _, sobs in pairs:
        sobs_open = bool(read_sobs_flags(sobs) & SOBS_FLAG_OPEN_RACK)
        if sobs_open != open_rack:
            raise SystemExit(
                f"{sobs} was generated with open_rack={sobs_open} but this run has "
                f"open_rack={open_rack}; regenerate under a dedicated tag"
            )
...
def main():
    ...
    p.add_argument(
        "--open-rack",
        action="store_true",
        help="the open-rack information condition: decode rows with the opponent-rack "
        "input block and require open-rack .sobs sidecars (a dedicated tag generated "
        "with generate_kill_test_data.py --open-rack)",
    )
    args = p.parse_args()
    ...
    build_cache(root / "slogs", cache, args.holdout_every, args.max_k, args.open_rack)
