#!/usr/bin/env python3
"""Minimal trainer for the pre-move value model (M_pre).

Trains M_pre to distill the teacher M_post over the candidate sets stored in a
directory of .pmt/.slog pairs (produced by the pre_move_value_target_generator),
reporting the top-K recall / rank-correlation metric against the teacher on a
held-out split. This is a lean, single-window loop for iterating on the model
and the metric; the generational generate->train pipeline and dashboard
integration (docs/roadmap2.md track A) are separate, later work.

Usage:
    ./py/scripts/pre_move_value/train.py --data-dir DIR [--holdout-dir DIR] \
...
from scribblez.pre_move_value.dataset import PmtDataset
from scribblez.pre_move_value.eval import evaluate
from scribblez.pre_move_value.model import PreMoveValueModel
from scribblez.pre_move_value.train_loop import LossConfig, run_epoch
from util.argparse_ext import ArgumentDefaultsHelpFormatter


def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Train the pre-move value model from .pmt/.slog pairs.",
        formatter_class=ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--data-dir", required=True, help="Directory of .pmt/.slog training pairs.")
    p.add_argument(
        "--holdout-dir",
        default=None,
        help="Directory of held-out .pmt/.slog pairs for the recall metric. "
    ...
    p.add_argument(
        "--contingent-features",
        action="store_true",
        help="Encode the full input layout (with contingent-draw features) for M_pre's board "
        "trunk. Selects the process-wide engine session's input arm; must be consistent across "
        "a run but is independent of the teacher's arm.",
    )
...
def main() -> int:
    ...
    train_ds = PmtDataset(args.data_dir)
    holdout_ds = PmtDataset(args.holdout_dir) if args.holdout_dir else train_ds
