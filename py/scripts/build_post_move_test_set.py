#!/usr/bin/env python3
"""Build (or re-score) the large post-move-value Monte-Carlo test set.

The dataset is a set of penultimate-bingo positions (see
docs/lexical_features_for_value.md and the post-move-value test-set README): each
...
low-variance "true value" the post-move-value model should predict.
...
Usage:
    py/scripts/build_post_move_test_set.py                    # full build
    py/scripts/build_post_move_test_set.py --count 1000 --games 10000
    py/scripts/build_post_move_test_set.py --skip-harvest     # re-score bundles
"""
...
def main() -> int:
    ...
    p.add_argument(
        "--dataset-name",
        default="post-move-value-test-dataset-large",
