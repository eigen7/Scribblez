...
The PR workflow in CLAUDE.md creates a worktree per task. A task abandoned
...
Run standalone, or via py/tools/gitea_serve.py, which prints the same report
every time it runs.
"""
...
def find_stale_worktrees(stale_days: float) -> list[StaleWorktree]:
...
def print_stale_report(stale_days: float = DEFAULT_STALE_DAYS):
    """Print abandoned-worktree candidates, or nothing if there are none.

    Called from py/tools/gitea_serve.py on every run; a session seeing this
    output should surface it to the user rather than delete anything itself.
    """
    stale = find_stale_worktrees(stale_days)
    if not stale:
        return
...
def main():
    args = get_args()
    if not find_stale_worktrees(args.days):
        print(f"No worktrees idle for {args.days:g}+ days.")
        return
    print_stale_report(args.days)
