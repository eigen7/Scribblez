...
Every change lands through a pull request on the local Gitea instance, which
the user reviews from the host browser (see py/tools/gitea_serve.py). This
tool wraps the whole lifecycle so a task is three commands:

  py/tools/pr.py worktree <branch>
      New worktree at /workspace/mount/worktrees/<branch> on a new branch
      <branch>, with submodules populated and a Claude commit identity, so the
      PR distinguishes Claude's commits from the user's.
  ...
  py/tools/pr.py create <branch> --title ... [--body-file ... | --body ...]
      ...
      credentials in /workspace/mount/gitea/claude_credentials.json) so Gitea
      shows Claude, not the reviewing admin, as pusher and author. Prints the
      review URL.
...
from tools.gitea_serve import GITEA_ROOT, REPO_NAME, REPO_ROOT, ensure_serving
from util.argparse_ext import ArgumentDefaultsHelpFormatter

WORKTREES_DIR = Path("/workspace/mount/worktrees")
...
def init_submodules(worktree: Path):
    ...
    are pushed upstream by the user, possibly long after the pointer bump; see
    submodules/devenv_utils/SUBMODULES.md). The main checkout always has the
    commit, so clone from it -- allowing direct-SHA fetch, since the pointer
    may be a detached commit there.
    """
...
def cmd_worktree(args: argparse.Namespace):
    path = WORKTREES_DIR / args.branch
    run(["git", "worktree", "add", str(path), "-b", args.branch], cwd=REPO_ROOT)
    # Worktrees don't inherit the main checkout's submodules.
    init_submodules(path)
    run(["git", "config", "extensions.worktreeConfig", "true"], cwd=REPO_ROOT)
...
def cmd_create(args: argparse.Namespace):
    admin, web_port, backend_port = ensure_serving()
    ...
    api(
        "PUT",
        backend_port,
        f"/repos/{owner}/{REPO_NAME}/collaborators/{CLAUDE_USER}",
        admin,
        {"permission": "write"},
    )
    ...
    push_url = (
        f"http://{CLAUDE_USER}:{claude['password']}@localhost:{backend_port}"
        f"/{owner}/{REPO_NAME}.git"
    )
    run(["git", "push", push_url, args.branch], cwd=REPO_ROOT)
    ...
    print(
        f"PR #{pr['number']}: http://localhost:{web_port}/{owner}/{REPO_NAME}/pulls/{pr['number']}"
    )


def cmd_merge(args: argparse.Namespace):
    admin, _, backend_port = ensure_serving()
    owner = admin["username"]
    pr = api("GET", backend_port, f"/repos/{owner}/{REPO_NAME}/pulls/{args.number}", admin)
    branch = pr["head"]["ref"]
    ...
    if not pr["merged"]:
        api(
            "POST",
            backend_port,
            f"/repos/{owner}/{REPO_NAME}/pulls/{args.number}/merge",
            admin,
            {"Do": "merge"},
        )
    run(["git", "pull", "--ff-only", "gitea", "main"], cwd=REPO_ROOT)
    api("DELETE", backend_port, f"/repos/{owner}/{REPO_NAME}/branches/{branch}", admin)
...
def parse_args() -> argparse.Namespace:
    ...
    p = sub.add_parser(
        "worktree",
        help="create a worktree + branch with a Claude identity",
    ...
    p = sub.add_parser(
        "create",
        help="push a branch and open its PR as the claude user",
        formatter_class=ArgumentDefaultsHelpFormatter,
    )
    ...
    p = sub.add_parser(
        "merge",
        help="merge an approved PR, fast-forward main, clean up branch + worktree",
        formatter_class=ArgumentDefaultsHelpFormatter,
    )
