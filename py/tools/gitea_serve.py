...
/workspace/mount/gitea, and the host browser reaches it at
http://localhost:<port>/ through the port published by run_docker.py.
...
that user, which is why setup_common.GITEA_PORT_ARGS publishes it bound to
127.0.0.1 on the host rather than 0.0.0.0. Gitea itself listens loopback-only
on <port>+1; the `gitea` remote targets it directly over basic auth, so git
operations work independently of nginx.

The script is idempotent; run it whenever the server is needed:
  ...
  1. First run: writes app.ini and nginx.conf, initializes the sqlite
     ...
     (mode 600) in /workspace/mount/gitea/admin_credentials.json. The web UI
     never asks for it; git over http uses it via the remote URL.
  2. Starts `gitea web` and nginx in the background unless already serving.
  3. Ensures /workspace/repo has a `gitea` remote and pushes main once so the
     server-side repo exists as a base for pull requests.
  4. Reports worktrees that look abandoned (see py/tools/stale_worktrees.py),
     since this script runs at least once per PR cycle.

To reset the instance entirely, stop the servers (`pkill -x gitea`,
`pkill -x nginx`) and delete /workspace/mount/gitea; the next run
...
from tools.stale_worktrees import print_stale_report
...
def remote_url(creds: dict, backend_port: int) -> str:
    return (
        f"http://{creds['username']}:{creds['password']}@localhost:{backend_port}"
        f"/{creds['username']}/{REPO_NAME}.git"
    )
...
def server_repo_exists(creds: dict, backend_port: int) -> bool:
    url = f"http://127.0.0.1:{backend_port}/api/v1/repos/{creds['username']}/{REPO_NAME}"
