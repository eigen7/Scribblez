"""Pre-move value model (M_pre): score a variable-size candidate set in one pass.

The model encodes the pre-move board once with the shared SpatialTrunk (the same
front end as the post-move model), then scores every candidate move against that
one encoding via cross-attention -- so the expensive board encode is amortized
across all candidates, not repeated per move (docs/roadmap.md, Phase 4).
...
carry a `pos_id` into [0, P). Each move:
  ...
  * is fused with its position's global summary and projected to the five
    ...
    (mean, std), matching the teacher readouts stored in the .pmt sidecar.

The heads predict what the teacher M_post would output for the candidate's
post-move state, from the mover's POV -- M_pre is a distillation of M_post over
the candidate set.
"""
...
class PreMoveValueModel(nn.Module):
...
def win_equity(probs: torch.Tensor) -> torch.Tensor:
    ...
    M_pre and the teacher are ranked the same way for the recall metric; the
    caller softmaxes M_pre's logits, while the teacher targets are already
    probabilities."""
