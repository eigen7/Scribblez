"""Move-feature inputs for the pre-move value model.

The encoding is owned by the engine
(engine/include/scribblez/pre_move_value_move_encoder.h) so the training dataset
and the eventual M_pre agent share one implementation and never drift. This
module is a thin Python view over it: `encode_moves` turns a (M,) array of
