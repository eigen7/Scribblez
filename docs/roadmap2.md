...
- **B1 — exact leave enumeration (keep-1/keep-2).** When the opponent's last
  move kept only 1–2 tiles, enumerate all possible leaves (≤ ~378), score
  each hypothesis by whether the observed move was near-best for it
  (forced-tile shadow search, ε-gap tolerance, uniform fallback), and sample
  from the resulting exact posterior. Covers ~22% of positions on top of the
  ~24% already exact (post-bingo / not-yet-acted).
- **B2 — accept/reject for the general case** (keep-3+): sample a rack from
  the pool and weight it by the observed move's plausibility under our own
  policy (equity-gap softmax) — Macondo's scheme, but falling back to the
  B1/B0-exact cases where available. The self-model assumption is exactly
  right for self-play data and the honest default against unknown opponents.
...
- **D1 — value-truncated rollouts** (design.md §5.2): sim a few plies, read
  ...
  late-vs-early phase gradient says this is the biggest sim-quality lever.
  Keep an **anchor fraction** of terminal rollouts per candidate — a
  ground-truth tether and a free measurement of the value model's bias. Costs
  to accept: `.sobs` artifacts become model-versioned, and sims start
  contending for the GPU (the contention-manager regime
  [generational_training.md](generational_training.md) plans for).
- **D2 — self-model plies.** Ply 1–2 of each rollout played by our full stack
  (move set evaluation top-1 or temperature sample, belief-sampled racks),
  then HastyBot to the horizon. This beats a generic policy upgrade because
  the evidence maps *read exactly plies 1–2*. Cost: batched leaf evaluation
  on the game-pool substrate; full-N passes at shallow plies only.
- **D3 — endgame solver for late-game rollouts.** Macondo's negamax solver
  has the needed shape (depth/time limits, first-win-only mode,
  transposition table, multithreading) but is Go, so this is a **port into
  the engine** (the WMP precedent). Rollouts switch to the solver when the
  bag empties, budget scaled by the root's distance to the end. Validate
  with a bag-≤-N kill-test slice and endgame-position match play.
...
- **E3 — the re-ranking experiment (step-6-lite).** The pivotal readout
  ...
  sims the promoted moves, and picks. Runnable with the kill-test's
  position-evaluation-based fusion model before the move set evaluation model
  exists; A5 productionizes it only if E3 says yes.
