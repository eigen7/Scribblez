...
- **Workload** — a kind of work, declared in the workload registry
  ...
  and names, per choice, the defaults that choice implies for other fields: the
  form renders it as a selector that starts unchosen and re-seeds those fields
  when picked, so a run cannot inherit a choice nobody made and the values that
  belong with it need not be remembered.
