...
A *blind spot* is a position where a play from outside HastyBot's static-equity
top moves out-sims all of them. The `blind_spots` dashboard workload collects
...
1. In the dashboard, pick the workload **Collect HastyBot blind spots**, create
   ...
   machines ([master_dashboard.md](master_dashboard.md), *SSH worker
   machines*). Ten rented CPU machines are ten workers; they need no
   coordination, since each plays its own randomly seeded games.
...
4. Browse what came in, then turn it into a committed directory:
   ...
   it can be rerun as the tag grows. `--min-gain` drops plays whose edge over
   the best top move is under that many win%: with 5000-rollout confirming
   sims the survey's own two-standard-error bar passes edges well under one.
...
- `survey/<stem>.simsurvey.json` -- one per game, slimmed to the positions
  found: per position the top moves and the outside picks with their
  confirming-sim summaries (`sim/rollout_summary.h`), and `positions_surveyed`,
  how many turns the game had, so rates stay computable.
- `gcg/<stem>-g0-turn<N>.gcg` -- the game up to each found position.

`<stem>` is `<timestamp>-<worker id>`, unique across workers. A game's full
survey (every legal play's screening summary, megabytes) stays on the worker
and is discarded; to keep it, run `py/scripts/sim_candidate_survey.py` on one
machine instead.
